...
class Train:
    def __init__(self,
        ...
        if gen_out_activation == 'sigmoid':
            self.gen = gen(out_activation=nn.Sigmoid).to(self.device)
            self.meta_gen = gen(out_activation=nn.Sigmoid).to(self.device)
        elif gen_out_activation == 'linear':
            self.gen = gen(out_activation=None).to(self.device)
            self.meta_gen = gen(out_activation=nn.Sigmoid).to(self.device)
        if dis_out_activation == 'sigmoid':
            self.dis = dis(out_activation=nn.Sigmoid, in_channel=self.n_pacgan).to(self.device)
            self.meta_dis = dis(out_activation=nn.Sigmoid, in_channel=self.n_pacgan).to(self.device)
        elif dis_out_activation == 'hardsigmoid':
            self.dis = dis(out_activation=nn.Hardsigmoid, in_channel=self.n_pacgan).to(self.device)
            self.meta_dis = dis(out_activation=nn.Sigmoid, in_channel=self.n_pacgan).to(self.device)
        elif dis_out_activation == 'linear':
            self.dis = dis(out_activation=None, in_channel=self.n_pacgan).to(self.device)
            self.meta_dis = dis(out_activation=nn.Sigmoid, in_channel=self.n_pacgan).to(self.device)
    ...
    def train(self, params_dict):
        ...
        gen.count_parameters()
        print("\nDiscriminator total parameters\n")
        dis.count_parameters()

        train_set = MetaMSDataset(self.mss_path_train, load_all=True)
        valid_set = MetaMSDataset(self.mss_path_valid, load_all=True)
        ...
        if self.load:
            self.meta_gen, self.meta_dis, optimizer_gen, optimizer_dis, epoch_offset = load_checkpoint(
        ...
        for epoch in range(epoch_offset, self.params['n_epochs']):
            ...
            for i, (mss, _) in enumerate(meta_train_loader):
                train_set = MSDataset(mss.squeeze(), log=True)
                ...
                train_loader = DataLoader(train_set,
                                          ...
                                          pin_memory=True,
                                          drop_last=True)
                gen.load_state_dict(self.meta_gen.state_dict())
                dis.load_state_dict(self.meta_dis.state_dict())
                gen.train()
                dis.train()
                for j, ms in enumerate(train_loader):
                    # pbar.update(1)
                    ms = torch.Tensor(ms.float()).to(self.device)
                    ms = torch.cat((ms, torch.zeros(ms.shape[0], 1).to(self.device)), 1)

                    if self.model_name == 'dcgan':
                        noise = torch.rand([self.params['bs'], 64, 1]).to(self.device)
                        fake_mss = gen(noise)
                    elif 'pacgan' in self.model_name or 'pacwgan' in self.model_name:
                        fake_mss = []
                        for _ in range(self.n_pacgan):
                            noise = torch.rand([int(self.params['bs'] / self.n_pacgan), 64, 1]).to(self.device)
                            fake_mss += [gen(noise).squeeze()]
                        fake_mss = torch.stack(fake_mss, 1)
                    elif self.model_name == 'gan':
                        noise = torch.rand([self.params['bs'], 100])
                        fake_mss = gen(noise.to(self.device))
                    if j == 0:
                        ...
                        try:
                            plt.savefig(
                                f"{self.params['checkpoint_path']}/{self.model_path}/spectra/spectrum_train_1_{epoch}")
                        except:
                    ...
                    if 'pacgan' in self.model_name or 'pacwgan' in self.model_name:
                        ...
                        dis_preds = dis(ms.unsqueeze(1))

                    ones = torch.ones(dis_preds.shape[0]).to(self.device)
                    ...
                    zeros = torch.zeros(dis_preds.shape[0]).to(self.device)
                    dis_fake_preds = dis(fake_mss.detach())
                    ...
                    for _ in range(3):
                        ...
                        if self.model_name == 'dcgan':
                            ...
                            noise = torch.rand([self.params['bs'], 100])
                            fake_mss = gen(noise.to(self.device))
                        dis_fake_preds = dis(fake_mss)
                        ones = torch.ones(fake_mss.shape[0]).to(self.device)
            ...
            self.meta_gen.meta_update(gen, meta_lr=0.01)
            self.meta_dis.meta_update(dis, meta_lr=0.01)
            ...
            dis.eval()
            with torch.no_grad():
                for i, (mss, _) in enumerate(meta_valid_loader):
                    valid_set = MSDataset(mss, log=True)
                    ...
                    valid_loader = DataLoader(valid_set,
                                              ...
                                              pin_memory=True,
                                              drop_last=True)
                    for j, ms in enumerate(valid_loader):
                        ms = torch.Tensor(ms.float()).to(self.device)
                        ms = torch.cat((ms, torch.zeros(ms.shape[0], 1).to(self.device)), 1)
                        ...
                        if self.model_name == 'dcgan':
                            ...
                            noise = torch.rand([self.params['bs'], 100]).to(self.device)
                            fake_mss = gen(noise)
                        if j == 0:
                            plt.plot(fake_mss.detach().cpu().numpy()[0][0])
                            plt.title(f"Epoch {epoch}")
                            ...
                            try:
                                plt.savefig(
                                    f"{self.params['checkpoint_path']}/{self.model_path}/spectra/spectrum_valid_1_{epoch}")
                            except:
                                print('error saving ms')
                            ...
                            try:
                                plt.savefig(
                                    f"{self.params['checkpoint_path']}/{self.model_path}/spectra/spectrum_valid_2_{epoch}")
                            except:
                                print('error saving ms')

                            plt.close()

                        targets = torch.ones(dis_preds.shape[0]).to(self.device)
                        loss_dis_real = self.criterion(
                        ...
                        dis_fake_preds = dis(fake_mss)
                        targets = torch.zeros(dis_preds.shape[0]).to(self.device)
                        loss_dis_fake = self.criterion(
                            dis_fake_preds.squeeze(),  # .view(len(batch), -1),
                            targets.long()
                        ) / self.params['bs']
    ...
    def get_l1(self, model, device):
        l1_reg = torch.tensor(0.).to(self.device)
        for name, param in model.named_parameters():
            if 'weight' in name:
                l1_reg += torch.norm(param, 1).to(self.device)
...
if __name__ == "__main__":
    ...
    if 'dcgan' in args.model:
        ...
        from src.models.pytorch.PacGAN_1D1 import Generator as gen
        from src.models.pytorch.PacGAN_1D1 import Discriminator as dis
