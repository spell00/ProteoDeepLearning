...
class Train:
    def __init__(self,
        ...
        self.model_name = f"{model_name}-{divergence}"
        if 'pacgan' in self.model_name:
            self.n_pacgan = int(self.model_name.split('pacgan')[1].split('-')[0])
        elif 'pacwgan' in self.model_name:
            self.n_pacgan = int(self.model_name.split('pacwgan')[1].split('-')[0])
        else:
            self.n_pacgan = None
        ...
        self.save = save
        try:
            self.criterion_g = VLOSS(divergence=divergence)
            self.criterion_d = QLOSS(divergence=divergence)
        except IOError:
            exit(f'{divergence} is not implemented. Valid choices are: [GAN, JSD, SQH, CHI, RKL, KLD]')
    ...
    def train(self, params_dict):
        ...
        train_set = MetaMSDataset(self.images_path_train, transform=False)
        valid_set = MetaMSDataset(self.images_path_valid, transform=False)
                ...
                for i, images in enumerate(meta_train_loader):
                    ...
                    for j, image in enumerate(train_loader):
                        ...
                        if j == 0:
                            ...
                            try:
                                plt.savefig(
                                    f"{self.params['checkpoint_path']}/spectra/spectrum_train_1_{epoch}")
                            except:
                            ...
                            try:
                                plt.savefig(
                                    f"{self.params['checkpoint_path']}/spectra/spectrum_train_2_{epoch}")
                            except:
                                print('error saving image')
                        ...
                        # if j % 2 == 0:
                        loss_dis_real.backward(retain_graph=True)
                        ...
                        del loss, loss_gen, loss_dis_fake, loss_dis_real, dis_preds, dis_fake_preds, noise
...
if __name__ == "__main__":
    ...
    parser.add_argument("--divergence", type=str, default="RKL", help="Divergence")
    parser.add_argument("--verbose", type=int, default=1)
    parser.add_argument("--gen_out_activation", type=str, default='sigmoid')
    parser.add_argument("--dis_out_activation", type=str, default='sigmoid')
    parser.add_argument("--model", type=str, default='pacgan1')
    ...
    best_parameters, values, experiment, model = optimize(
        parameters=[
            ...
            {"name": "learning_rate", "type": "range", "bounds": [1e-3, 1e-2], "log_scale": True},
        ],
