...
class Train:
    def __init__(self,
        ...
        if dis_out_activation == 'sigmoid':
            self.dis = dis(out_activation=nn.Sigmoid, in_channel=self.n_pacgan)
        elif dis_out_activation == 'hardsigmoid':
            self.dis = dis(out_activation=nn.Hardsigmoid, in_channel=self.n_pacgan)
        elif dis_out_activation == 'linear':
            self.dis = dis(out_activation=None, in_channel=self.n_pacgan)
            ...
            for epoch in range(epoch_offset, self.params['n_epochs']):
                ...
                for i, (mss, _) in enumerate(meta_train_loader):
                    ...
                    for j, ms in enumerate(train_loader):
                        ...
                        ms = torch.cat((ms, torch.zeros(ms.shape[0], 1).to(device)), 1)
