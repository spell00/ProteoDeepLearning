...
class Train:
    ...
    def train(self, params_dict):
        ...
        pca.fit(samples.reshape([samples.shape[1], -1]))
        ...
        self.optimizer_dis.param_groups[0]['lr'] = self.params['lr'] / 10
        ...
        for epoch in range(epoch_offset, self.params['n_epochs']):
            ...
            for i, (mss, _, columns) in enumerate(meta_train_loader):
                ...
                for j, (ms, pmz, tics, rtime) in enumerate(train_loader):
                    ...
                    if j % self.n_dis == 0:
                        ...
                        if (epoch - 1) % 100 == 0:
                            fid = calculate_fid(ms.detach().cpu().reshape(-1, ms.shape[-1]),
            ...
            for i, (mss, _, columns) in enumerate(meta_valid_loader):
                ...
                for j, (ms, pmz, tics, rtime) in enumerate(valid_loader):
                    ms = torch.Tensor(ms.float()).to(device)
                    dis_real_preds, h_real = self.get_real_tensor(ms, forget_pacgan=False)
                    fake_mss = self.get_fake_tensor(forget_pacgan=False)
                    if j == 0:
                        if self.save:
            ...
            if self.params['scheduler'] == "ReduceLROnPlateau":
                if epoch - epoch_offset > 5:
                    lr_scheduler_gen.step(abs(results['losses_gen']["valid"][-1]))
                    lr_scheduler_dis.step(abs(results['losses_dis_fake']["valid"][-1]))
            if self.save:
            ...
            if (np.abs(results['fake_nll']['valid'][-1]) < np.abs(best_dict['fake_nll']) or best_dict[
                'loss'] == -1) and not np.isnan(
                results['fake_nll']['valid'][-1]):
                if self.verbose > 1:
                    print('BEST EPOCH!', results['fake_nll']['valid'][-1])
                early_stop_counter = 0
        ...
        for _ in range(n):
            ...
            if self.unroll_nsteps > 0:
                ...
                for i in range(self.unroll_nsteps):
                    ...
                    dis_fake_preds, h_fake = self.dis(fake_mss.detach())
                    dis_real_preds, h_real = self.get_real_tensor(real_ms.detach(), forget_pacgan=False)
...
if __name__ == "__main__":
    ...
    params = {
        ...
        "z_dim": None,
        "scale": 1,
        "n_res_channel": 0,  # TODO resnet not implemented
        "n_res": 0,  # TODO resnet not implemented
        "dropout": 0.5,
        "n_channel": 512,
        "n_embed": 1024,
        "l1": 0,
